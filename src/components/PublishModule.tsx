  ...
  const handleDownloadEpub = async () => {
    if (!isBookComplete) {
      const proceed = confirm(
        'Warning: Some chapters do not yet have complete prose. Velora recommends publishing only 100% complete works. Do you want to export anyway?'
      );
      if (!proceed) return;
    }
    ...
    try {
      ...
      const filename = `${project.title.toLowerCase().replace(/[^a-z0-9]+/g, '_')}_velora.epub`;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Production-grade packaging for EPUB 3.0 e-readers, print-ready PDF folio, and editable DOCX formats.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left: Export Formats & Gates (5 cols) */}
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-6 space-y-4">
            ...
            <div className="space-y-2.5 text-xs">
              <div className="flex items-center justify-between p-2 rounded bg-slate-950 border border-slate-800">
                ...
                <span className="font-mono font-bold text-slate-100">
                  {validChapters.length} / {project.chapters.length}
                </span>
              </div>
            ...
            {!isBookComplete && (
              <div className="p-3 bg-amber-500/10 border border-amber-500/30 rounded-lg text-xs text-amber-200 space-y-2">
                <p>
                  Some chapters do not yet have generated literary prose. You can trigger batch creation in the Forge Orchestrator:
                </p>
                <button
                  type="button"
                  onClick={() => onNavigateToModule('forge')}
                  className="px-3 py-1 rounded bg-amber-500 text-slate-950 font-medium text-xs flex items-center gap-1"
                >
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-6 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full p-4 rounded-xl border border-amber-500/40 bg-gradient-to-r from-amber-500/15 to-amber-600/10 hover:bg-amber-500/20 text-left transition-all flex items-center justify-between shadow-sm group"
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-amber-200">
                      Standard EPUB 3.0 E-Book
                    </span>
                  ...
                  <p className="text-xs text-slate-400">
                    Valid container for Apple Books, Kindle (via Send-to-Kindle), Kobo, and Calibre.
                  </p>
                </div>
              ...
              <button
                ...
                className="w-full p-4 rounded-xl border border-slate-800 bg-slate-950 hover:border-slate-700 text-left transition-all flex items-center justify-between group"
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-slate-200">
                      Print-Ready PDF / Folio
                    </span>
                  ...
                  <p className="text-xs text-slate-400">
                    Clean printable layout with page breaks, front matter, and book typography.
                  </p>
                </div>
              ...
              <button
                type="button"
                id="btn-export-docx"
                onClick={() => exportProjectToDocxHtml(project)}
                className="w-full p-3 rounded-xl border border-slate-800 bg-slate-950 hover:border-slate-700 text-left transition-all flex items-center justify-between group"
              >
                <div className="space-y-0.5">
                  ...
                  <p className="text-[11px] text-slate-400">
                    Editable manuscript with standardized 1-inch margins and page breaks.
                  </p>
                </div>
              ...
              >
                <div className="space-y-0.5">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-slate-300">
                      Full Velora Project JSON Archive
                    </span>
                    <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-slate-800 text-slate-400">
                      .json
                    </span>
                  ...
                  <p className="text-[11px] text-slate-400">
                    Includes Story Bible, Characters, Timeline, Cover Config, and Manuscript.
                  </p>
                </div>
        ...
        {/* Right: Master Book Preview Sheet (7 cols) */}
        <div className="lg:col-span-7 bg-slate-900/60 border border-slate-800 rounded-2xl p-8 space-y-8">
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            ...
            <span className="text-xs text-slate-400 font-mono">
              {validChapters.length} Chapters &bull; {totalWords.toLocaleString()} Words
            </span>
          </div>

          {/* Front Matter Display */}
          <div className="text-center py-8 space-y-4 bg-slate-950/60 rounded-xl border border-slate-800/80 p-6">
            <h1 className="text-3xl font-serif font-bold tracking-wider uppercase text-slate-100">
              {project.title}
            </h1>
            {project.subtitle && (
              <p className="text-sm italic text-amber-300 font-serif">
                {project.subtitle}
              </p>
            )}
            <div className="pt-4">
              <span className="text-xs tracking-[0.25em] uppercase font-mono text-slate-400">
                A NOVEL BY
              </span>
              <p className="text-base font-serif font-bold text-slate-200 mt-1">
                {project.author}
              </p>
            </div>
            <div className="text-[11px] text-slate-500 font-mono pt-4 border-t border-slate-800/60">
              First Edition &bull; Published via {brand} Platform &bull; {new Date().getFullYear()}
            </div>
          </div>
              ...
              {project.chapters.map((c) => (
                <div key={c.chapterNumber} className="py-2.5 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="font-mono text-amber-400/80 font-bold">
                      Chapter {c.chapterNumber}
                    </span>
                    <span className="text-slate-200 font-serif">{c.title}</span>
                  </div>
                  <span className="text-[11px] font-mono text-slate-500">
                    {c.wordCount > 0 ? `${c.wordCount} w` : 'Pending'}
                  </span>
                </div>
              ))}
            </div>
